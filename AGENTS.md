...
5. **Артефакты сборки**: не коммитить содержимое `target/`; при локальной проверке оно пересоздаётся Maven.
6. **Стиль правок**: минимальный дифф под задачу; повторять существующие паттерны именования; boilerplate, Lombok и MapStruct — по правилам [раздела выше](#boilerplate-lombok-mapstruct).
