...
В обычном режиме starter пишет только события уровней `INFO`, `WARN` и `ERROR`. Для подробной диагностики можно временно включить `DEBUG` для пакета библиотеки:

```yaml
logging:
  level:
    ru.sber.rcln.reflex.telemetry: DEBUG
```
