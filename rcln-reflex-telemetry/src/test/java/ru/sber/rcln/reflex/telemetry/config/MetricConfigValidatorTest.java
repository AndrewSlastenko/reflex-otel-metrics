...
class MetricConfigValidatorTest {
    ...
    @Test
    void shouldRejectJdbcQuerySchemaWithInvalidIdentifier() {
        ResolvedMetricConfig base = baseConfig(ReflexTelemetryProperties.MetricSourceType.JDBC, MetricKind.GAUGE);
        ResolvedMetricConfig config = new ResolvedMetricConfig(
                base.metricId(), base.source(), base.enabled(), base.exportedMetricName(), base.name(),
                base.scope(), base.description(), base.unit(), base.attributes(), base.dataSourceRef(),
                base.metricKind(), base.schedule(), base.timeout(), base.lockAtMostFor(), base.lockAtLeastFor(),
                base.maxSeries(), base.overflowPolicy(), base.histogramBuckets(), "documents; drop table foo");

        assertThat(new MetricConfigValidator().validate(config))
                .containsExactly("Metric 'documents-by-status' requires query.schema to be a simple SQL identifier "
                        + "(letters, digits, underscore; not starting with a digit)");
    }

    @Test
    void shouldAcceptJdbcQuerySchemaWithSimpleIdentifier() {
        ResolvedMetricConfig base = baseConfig(ReflexTelemetryProperties.MetricSourceType.JDBC, MetricKind.GAUGE);
        ResolvedMetricConfig config = new ResolvedMetricConfig(
                base.metricId(), base.source(), base.enabled(), base.exportedMetricName(), base.name(),
                base.scope(), base.description(), base.unit(), base.attributes(), base.dataSourceRef(),
                base.metricKind(), base.schedule(), base.timeout(), base.lockAtMostFor(), base.lockAtLeastFor(),
                base.maxSeries(), base.overflowPolicy(), base.histogramBuckets(), "documents_v2");

        assertThat(new MetricConfigValidator().validate(config)).isEmpty();
    }

    @Test
    void shouldRejectManualMetricWithQuerySchema() {
        ResolvedMetricConfig base = baseConfig(ReflexTelemetryProperties.MetricSourceType.MANUAL, MetricKind.COUNTER);
        ResolvedMetricConfig config = new ResolvedMetricConfig(
                base.metricId(), base.source(), base.enabled(), base.exportedMetricName(), base.name(),
                base.scope(), base.description(), base.unit(), base.attributes(), null,
                base.metricKind(), base.schedule(), base.timeout(), base.lockAtMostFor(), base.lockAtLeastFor(),
                base.maxSeries(), base.overflowPolicy(), base.histogramBuckets(), "documents");

        assertThat(new MetricConfigValidator().validate(config))
                .containsExactly("Metric 'documents-by-status' must not set query.schema for MANUAL source");
    }
